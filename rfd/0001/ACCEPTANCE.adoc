...
The `login-output-smoke` check launches the package against a two-output
headless Sway session and verifies that authentication renders on the selected
output while the other output keeps unobstructed wallpaper, including output
removal, re-enable, mode change, and caret and input preservation across the
migration.
...
packaged capture-only preview output. `make check-rfds` verifies the exact manifest,
